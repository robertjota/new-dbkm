...
class Recurso extends ActiveRecord
{
    ...
    {
        $columns = 'recurso.modulo, count(recurso.id) as total ';
        $conditions = 'recurso.id IS NOT NULL AND recurso.id > 1';
        if ($estado != 'todos') {
            $conditions .= ($estado == self::ACTIVO) ? " AND activo=" . self::ACTIVO : " AND activo=" . self::INACTIVO;
        }
        return $this->find("columns: $columns", "conditions: $conditions", "group: recurso.modulo", "order: recurso.modulo ASC");
    }
